...
handlers._tokens.post = function (data, callback) {
	...
	if (phone && password) {
		...
		_performance.mark('beginning user lookups');
		_data.read('users', phone, function (err, userData) {
			...
			if (!err && userData) {
				...
				if (hashedPassword == userData.hashedPassword) {
					...
					_data.create('tokens', tokenId, tokenObject, function (err, data) {
						...
						_performance.measure('validating user input','entered function','storing token complete','inputs validated');
						_performance.measure('user lookup','beginning user lookups','user lookup complete');
						_performance.measure('password hashing','beginning password hashing','password hashing complete');
						_performance.measure('token data creation','creating data for the token','beginning token store');
						_performance.measure('token storing','beginning token store','storing token complete');
                        ...
                        if(userChecks.length<config.maxChecks){
							...
							dns.resolve(hostName,function(err,records){
								if(!err && records){
									...
									callback(400,{'Error':'the host name for the url entered did not resolve to any dns entries'});
								}
							});
